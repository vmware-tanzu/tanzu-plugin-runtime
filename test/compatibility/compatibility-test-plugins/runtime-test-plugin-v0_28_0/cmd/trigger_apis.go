...
func triggerContextAPIs(api *core.API, logs map[core.RuntimeAPIName][]core.APILog) {
	...
	case core.RemoveContextAPIName:
		log := triggerDeleteContextAPI(api)
	...
	case core.RemoveCurrentContextAPIName:
		log := triggerRemoveCurrentContextAPI(api)
		logs[core.RemoveCurrentContextAPIName] = append(logs[core.RemoveCurrentContextAPIName], log)
