...
const (
	...
	// IgnoreAnsiCodes ignores ANSI color code when calculating column widths.
	IgnoreAnsiCodes
)
...
func (b *Writer) Init(output io.Writer, minwidth, tabwidth, padding int, padchar byte, paddingStart int, flags uint) *Writer {
	if minwidth < 0 || tabwidth < 0 || padding < 0 || paddingStart < 0 {
...
func (b *Writer) updateWidth() {
	content := string(b.buf[b.pos:])
	if b.flags&IgnoreAnsiCodes != 0 {
		content = stripansi.Strip(content)
	}
	b.cell.width += utf8.RuneCountInString(content)
	b.pos = len(b.buf)
}
